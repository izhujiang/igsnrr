...
function fetchAndSaveData(areaCode, type, dataTime) {
	...
	const filename = path.join(
		directoryPath,
		`${areaCode}_${type}_${formattedDate}.json`,
	);
...
async function fetchAndSaveDataRange(areaCode, startTime, endTime) {
	...
	while (currentTime <= end) {
		...
		fetchAndSaveData(areaCode, "TEMP", formattedTime);

		randomSeconds = Math.floor(1 + Math.random() * max_interval);
		console.log(`Sleeping for ${randomSeconds} seconds...`);
		await randomSleep(randomSeconds * 1000);
		fetchAndSaveData(areaCode, "RAIN", formattedTime);

		randomSeconds = Math.floor(1 + Math.random() * max_interval);
		console.log(`Sleeping for ${randomSeconds} seconds...`);
		await randomSleep(randomSeconds * 1000);
		fetchAndSaveData(areaCode, "WIND", formattedTime);

		randomSeconds = Math.floor(1 + Math.random() * max_interval);
		console.log(`Sleeping for ${randomSeconds} seconds...`);
		await randomSleep(randomSeconds * 1000);
		fetchAndSaveData(areaCode, "Press", formattedTime);

		randomSeconds = Math.floor(1 + Math.random() * max_interval);
		console.log(`Sleeping for ${randomSeconds} seconds...`);
		await randomSleep(randomSeconds * 1000);
		fetchAndSaveData(areaCode, "RH", formattedTime);

		randomSeconds = Math.floor(1 + Math.random() * max_interval);
		console.log(`Sleeping for ${randomSeconds} seconds...`);
		await randomSleep(randomSeconds * 1000);
		fetchAndSaveData(areaCode, "VIS", formattedTime);
...
// fetchAndSaveDataRange("52267", "2025-01-14 00:00", "2025-01-15 01:00");
//
//
...
const areaCodes = ["52267", "93602", "52576", "52607"];
// 52267  -- 额济纳旗
// 93602  -- 阿左旗
// 52576  -- 阿右旗
// 52607  -- 阿拉善高新区
if (inputArg == "auto") {
	...
	areaCodes.forEach(function (areaCode) {
		console.log(areaCode, startOfYesterday, endOfYesterday);
		fetchAndSaveDataRange(areaCode, startOfYesterday, endOfYesterday);
	});
} else {
	// modify range and run manually
	// dataTime format must be: "YYYY-MM-DD HH:mm" like "2025-01-12 00:00"
	data_range_start = "2025-04-29 00:00";
	data_range_end = "2025-04-30 01:00";
	areaCodes.forEach(function (areaCode) {
		fetchAndSaveDataRange(areaCode, data_range_start, data_range_end);
	});
}